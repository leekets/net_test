...
var c_next = 1;	//往后跳的数量
var c_group = 0;
var c_mode = 1; //0:lrc mode; 1:normal mode; 3: back to index  ////url 选择测试的模式
$(function () {
    ...
    var baseurl2 = baseurl + "TestArr";
    if (cateJSON[c] != undefined) {
        if (cateJSON[a].parentID == cateJSON[c].parentID) {
            if (c_mode == 1) {
                if (pageSize > comments.length) {
                    //alert("1");
                    jumpUrl = baseurl2 + "?id=" + cateJSON[c].ID + "&pageSize=1&c_audio=" + c_audio + "&c_mode=" + c_mode;
                } else {
                    //alert("2");
                    pageSize = Number(pageSize) + Number(c_nextSize);
                    jumpUrl = baseurl2 + "?id=" + cateJSON[a].ID + "&pageSize=" + pageSize + "&c_audio=" + c_audio + "&c_mode=" + c_mode; //jump to the beginning and pagesize+1
                }
            }
            if (c_mode == 0) {
                jumpUrl = baseurl2 + "?id=" + cateJSON[c].ID + "&pageSize=" + pageSize + "&c_audio=" + c_audio + "&c_mode=" + c_mode; //jump to next question category
            }
            if (c_mode == 2) {
                jumpUrl = baseurl2 + "?id=" + cateJSON[c].ID + "&pageSize=" + pageSize + "&c_audio=" + c_audio + "&c_mode=" + c_mode; //jump to next question catego	
            }
            if (c_mode == 3) {
                jumpUrl = jumpIndex;
            }
    ...
    if (action == 1) {
        jumpUrl = baseurl2 + "?id=" + x + "&pageSize=" + (Number(pageSize) + Number(c_next)) + "&c_audio=" + c_audio + "&action=1" + "&c_mode=" + c_mode;
    }
    ...
    function act() {
        ...
        if (index < comments.length) {					//如果指针在范围内；
            ...
            $('#mainInput').bind('focus keyup input paste', function () {  //采用几个事件来触发（已增加鼠标粘贴事件）
                var lenNow = $("#mainInput").val().length;
                if (lenNow > lenMAX) {
